        ...
        if(count==0): #只需關心買入條件 
            for key in buylist:
                ...
                if(key=='波動率'): #波動率
                    ...
                    if(buylist[key][0]=='週'):
                        if(weeknumber==0): #不用檢查前一個  
                            if(isFloat(newweek[weekcount][25])!=False):
                                number1=0.01*int(buylist[key][1])
                                number2=0.01*int(buylist[key][2])
                    ...
                    if(buylist[key][0]=='月'):
                        if(weeknumber==0): #不用檢查前一個  
                            if(isFloat(isFloat(newmonth[monthcount][25]))!='#'):
                                number1=0.01*int(buylist[key][1])
                                number2=0.01*int(buylist[key][2])
                            ...
                            if(monthlist[weeknumber]!=monthlist[weeknumber-1]):  #檢查前一個的週別是否一樣，一樣就不用再做判斷
                                if(isFloat(newmonth[monthcount][25])!='#'):
                                    number1=0.01*int(buylist[key][1])
                                    number2=0.01*int(buylist[key][2])
                                    if(float(newmonth[monthcount][25])>number1 and float(newmonth[monthcount][25])<number2):
                                        buydaysmemory[counter]=11 
            ...
            for key in selllist:
                ...
                if(key=='波動率'): #波動率
                    ...
                    if(selllist[key][0]=='月'):
                        if(weeknumber==0): #不用檢查前一個  
                            if(isFloat(newmonth[monthcount][25])!=False):
                                number1=0.01*int(selllist[key][1])
                                number2=0.01*int(selllist[key][2])
                                if(float(newmonth[monthcount][25])>number1 and float(newmonth[monthcount][25])<number2):
                                    selldaysmemory[counter]=11
    ...
    if(len(money)==1):
        ...
        if(len(money)%2==0): #偶數
            for i in range(1,len(money),2):
                if(isFloat(money[i])!=False and isFloat(money[i-1])!=False):
...
def KD(operator,value,days,getvalue):
    ...
    if(isFloat(getvalue)==False or isFloat(value)==False):
        return 0
    elif(operator=='>'):
...
def GD(results,newlistvalue,value1,value2,number1,number2,key):#資料list、要確認的日期、要確認的值
    for i in range(len(results)):
        if(newlistvalue==results[i][1]):
            ...
            break
    if(isFloat(temp1)==False or isFloat(temp2)==False or isFloat(value1)==False or isFloat(value2)==False or float(value1)!=float(value2)):
        return 12
...
def Place(operator,MACDvalue,DIFvalue):
    if(isFloat(MACDvalue) == False or isFloat(DIFvalue)==False):
        return 12
    elif(operator=='>'):
...
def VALUE(results,newlistvalue,number5,number20,number60,value5,value20,value60,key,gd):
    for i in range(len(results)):
        if(newlistvalue==results[i][1]):
            ...
            break
    if(key=='5/20' and float(number5)==float(number20) and isFloat(temp1)!= False and isFloat(temp2)!=False and isFloat(number5) !=False and isFloat(number20) !=False): #5/20
        if(gd==0): #黃金
        ...
        return 12
    if(key=='5/60' and float(number5)==float(number60) and isFloat(temp1)!= False and isFloat(temp3)!=False and isFloat(number5) !=False and isFloat(number60) !=False): #5/60
        if(gd==0): #黃金
        ...
        return 12
    if(key=='20/60' and float(number20)==float(number60) and isFloat(temp2)!= False and isFloat(temp3)!=False and isFloat(number20) !=False and isFloat(number60) !=False): #20/60
        if(gd==0): #黃金
...
def VOLUME(results,valuedate,number,value,days,key):
    for i in range(len(results)):
        if(valuedate==results[i][1]):
            ...
            break
    if(isFloat(temp1)!=False and isFloat(value)!=False):
        if(key=='增加'): #增加
...
def boolean(key,date,results,ma20,mbnumber,days):
    ...
    for i in range(19):
        if(isFloat(results[b][7])==False or isFloat(ma20)==False):
            if(key=='跌破' or key=='突破'):
                return 12
            elif(key=='變大' or key=='變小'):
                return 0
            ...
            b-=1
        if(isFloat(results[yesterdayb][7])==False or isFloat(yesterdayma20)==False):
            if(key=='跌破' or key=='突破'):
                return 12
            elif(key=='變大' or key=='變小'):
                return 0
    ...
    if(isFloat(results[position][7])!=False and isFloat(yesterdaygmoney)!=False):
