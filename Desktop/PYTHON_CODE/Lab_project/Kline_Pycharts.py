...
class SelectMainWindow(QtWidgets.QMainWindow, Ui_MainPage):
	...
	def __init__(self, parent=None): 
		...
		self.sma_5 = talib.SMA(np.array(self.close_pr_market,dtype=float), 5) #製作MA5
		self.sma_20 = talib.SMA(np.array(self.close_pr_market,dtype=float), 20) #製作MA20
		self.sma_60 = talib.SMA(np.array(self.close_pr_market,dtype=float), 60) #製作MA60
		self.H_line,self.M_line,self.L_line=talib.BBANDS(np.array(self.close_pr_market,dtype=float), timeperiod=20, nbdevup=2, nbdevdn=2, matype=0)#製作布林通道
...
self.checkBox_28,self.checkBox_29,self.checkBox_30]
	...
	def master_description_text(self):
		...
		elif self.listWidget.currentItem().text() == '20週均線突破':
	...
	def sort_info_market(self,pr_list,sql_pr):
		self.market_value = 'SELECT '+sql_pr+' FROM TAIEX ORDER BY TradeDate DESC LIMIT 180'
	...
	def smart_stock(self):
		...
		self.tableWidget_5.setRowCount(len(check_prediction_stock[0]))
		check_pd_count = 0
		for check_list in check_prediction_stock[0]:
			check_prediction_info = '''SELECT sid,TradeDate,TradeValue,TradeVolume,OpeningPrice,HighestPrice,LowestPrice,ClosingPrice,Change_ FROM DayStockInformation WHERE sid=%s ORDER BY TradeDate DESC'''
	...
	def collect_condition(self): #將條件整理好並送入filter8.py裡
		...
		for select in self.select_date_list:
			...
			try:
				...
				pass
		try:
			if self.comboBox_3.currentText() in ['成交資料(日)']:
				total_output.append(['T0','New',filter8.IsLastDay()])
				self.tableWidget.setColumnCount(8)
				for i in range(0,8):
					total = ['股票代碼','股票名稱','開盤價','最高價','最低價','收盤價','漲跌價差','成交筆數']
					newItem = QTableWidgetItem(total[i])
			...
			elif self.comboBox_3.currentText() in ['成交資料(週)']:
				total_output.append(['T0','NewWeek',filter8.IsLastWeek()])
				self.tableWidget.setColumnCount(8)
				for i in range(0,8):
					total = ['股票代碼','股票名稱','開盤價','最高價','最低價','收盤價','漲跌價差','成交筆數']
					newItem = QTableWidgetItem(total[i])
					...
					stylesheet = "::section{Background-color:'#2894FF'; font:15px; color:white;}"
					self.tableWidget.horizontalHeader().setStyleSheet(stylesheet)
			elif self.comboBox_3.currentText() in ['成交資料(月)']:
				total_output.append(['T0','NewMonth',filter8.IsLastMonth()])
				self.tableWidget.setColumnCount(8)
				for i in range(0,8):
					total = ['股票代碼','股票名稱','開盤價','最高價','最低價','收盤價','漲跌價差','成交筆數']
					newItem = QTableWidgetItem(total[i])
				...
				for i in range(0,10):
					total = ['股票代碼','股票名稱','收盤價','漲跌價差','RSI6(日)','RSI9(日)','RSI6(週)','RSI9(週)','RSI6(月)','RSI9(月)']
					newItem = QTableWidgetItem(total[i])
					self.tableWidget.setHorizontalHeaderItem(i,newItem)
					stylesheet = "::section{Background-color:'#2894FF'; font:15px; color:white;}"
					self.tableWidget.horizontalHeader().setStyleSheet(stylesheet)
				...
				self.tableWidget.setColumnCount(10)
				for i in range(0,10):
					total = ['股票代碼','股票名稱','收盤價','漲跌價差','K(日)','D(日)','K(週)','D(週)','K(月)','D(月)']
					newItem = QTableWidgetItem(total[i])
					self.tableWidget.setHorizontalHeaderItem(i,newItem)
					stylesheet = "::section{Background-color:'#2894FF'; font:15px; color:white;}"
					self.tableWidget.horizontalHeader().setStyleSheet(stylesheet)
				...
				self.tableWidget.setColumnCount(10)
				for i in range(0,10):
					total = ['股票代碼','股票名稱','收盤價','漲跌價差','MACD(日)','DIF(日)','MACD(週)','DIF(週)','MACD(月)','DIF(月)']
					newItem = QTableWidgetItem(total[i])
					self.tableWidget.setHorizontalHeaderItem(i,newItem)
					stylesheet = "::section{Background-color:'#2894FF'; font:15px; color:white;}"
					self.tableWidget.horizontalHeader().setStyleSheet(stylesheet)
				...
				self.tableWidget.setColumnCount(10)
				for i in range(0,10):
					total = ['股票代碼','股票名稱','收盤價','漲跌價差','法人買賣超' + '\n' + '外資不含自營','法人買賣超' + '\n' + '外資+自營','法人買賣超' + '\n' + '投信','法人買賣超' + '\n' + '自營商' + '\n' + '自行買賣','法人買賣超' + '\n' + '自營商_避險','總和']
					newItem = QTableWidgetItem(total[i])
					self.tableWidget.setHorizontalHeaderItem(i,newItem)
					stylesheet = "::section{Background-color:'#2894FF'; font:15px; color:white;}"
					self.tableWidget.horizontalHeader().setStyleSheet(stylesheet)
			...
			count = 0
			self.tableWidget.setRowCount(len(self.select_info)-1)
			for info in list(self.select_info): #將條件widget放入table裡
				infos = list(info)
				del infos[2]
				print(infos)
				for i in range(0,int(self.tableWidget.columnCount())):
					newItem = QTableWidgetItem(str(infos[i]))
					textFont = QFont("song", 12, QFont.Bold)  
					newItem.setTextAlignment(Qt.AlignHCenter |  Qt.AlignVCenter)
					newItem.setFont(textFont)
					newItem.setFlags(QtCore.Qt.ItemIsEnabled)
...
#############################################################################################################################################
	...
	def market_kline(self):
		self.kline = (
			...
			.set_global_opts(
				datazoom_opts = [opts.DataZoomOpts(type_="slider",pos_bottom='bottom',xaxis_index=[0, 1, 2, 3],range_start=60,range_end=100),opts.DataZoomOpts(type_="inside",xaxis_index=[0, 1, 2, 3],)],
...
#################################以下為均線################################################
		self.line_average_MA5 = (
			...
			.add_yaxis(
				...
				y_axis=self.sma_5,
				is_smooth=True,
				linestyle_opts=opts.LineStyleOpts(opacity=0.5,width=2.5),
		...
		self.bar_macd = (
			...
			.set_global_opts(
				xaxis_opts = opts.AxisOpts(
					is_show=True,
					splitline_opts = opts.SplitLineOpts(is_show=True),
					axislabel_opts = opts.LabelOpts(is_show=True if str(self.comboBox_11.currentText()) == "MACD" else False),
					axistick_opts = opts.AxisTickOpts(is_show=False),
				),
				legend_opts = opts.LegendOpts(is_show=False, pos_bottom=10, pos_left="center"),
				yaxis_opts=opts.AxisOpts(
...
#################################以下為MACD & DIF################################################
		...
		if self.comboBox_13.currentText() != self.comboBox_12.currentText() and self.comboBox_13.currentText() != self.comboBox_11.currentText() and self.comboBox_12.currentText() != self.comboBox_11.currentText():
			...
			opendata = list()
			closedata = list()
			for opens in self.open_pr_market:
				opendata.append(float(opens))
			for close in self.close_pr_market:
...
class PyechartsMainWindow(QtWidgets.QMainWindow, Ui_Pyechart):
	...
	def sort_info(self,pr_list,sql_pr,num,date_sql,date_type):
		no_show = list()
		count = 0
		check_count = 0
		show_list = ['ClosingPrice','OpeningPrice','HighestPrice','LowestPrice']
		if sql_pr in show_list:
			self.search_close = 'SELECT '+sql_pr+' FROM '+date_sql+' where sid=%s ORDER BY '+date_type+' DESC LIMIT 180'
			...
			self.search_close = 'SELECT ClosingPrice FROM '+date_sql+' where sid=%s ORDER BY '+date_type+' DESC LIMIT 180'
			self.cursor.execute(self.search_close,num)
	...
	#################################以下為資料讀取###########################################
			...
			if stock_text:
				try:
					...
					self.sort_info(close_pr,'ClosingPrice',number,date_status,date_type) #收盤
					self.sort_info(open_pr,'OpeningPrice',number,date_status,date_type) #開盤
					self.sort_info(high_pr,'HighestPrice',number,date_status,date_type) #最高價
					self.sort_info(low_pr,'LowestPrice',number,date_status,date_type) #最低價
					if date_status == 'DayStockInformation':
						self.sort_info(dates,'TradeDate',number,date_status,date_type) #日期
					elif date_status == 'WeekStockInformation':
						self.sort_info(dates,'StartDate',number,date_status,date_type) #日期
					elif date_status == 'MonthStockInformation':
						self.sort_info(dates,'StartDate',number,date_status,date_type) #日期
					self.sort_info(vols,'TradeVolume',number,date_status,date_type) #成交量
					self.sort_info(macd,'MACD9',number,date_status,date_type) #macd
					self.sort_info(rsi,'RSI6',number,date_status,date_type) #rsi
					self.sort_info(k_value,'K9_',number,date_status,date_type) #k_value
					self.sort_info(d_value,'D9',number,date_status,date_type) #d_value
					self.sort_info(dif,'DIF12and26',number,date_status,date_type) #dif
					...
					self.sort_info(close_pr,'ClosingPrice',stock_text,date_status,date_type) #收盤
					self.sort_info(open_pr,'OpeningPrice',stock_text,date_status,date_type) #開盤
					self.sort_info(high_pr,'HighestPrice',stock_text,date_status,date_type) #最高價
					self.sort_info(low_pr,'LowestPrice',stock_text,date_status,date_type) #最低價
					if date_status == 'DayStockInformation':
						self.sort_info(dates,'TradeDate',stock_text,date_status,date_type) #日期
					elif date_status == 'WeekStockInformation':
						self.sort_info(dates,'StartDate',stock_text,date_status,date_type) #日期
					elif date_status == 'MonthStockInformation':
						self.sort_info(dates,'StartDate',stock_text,date_status,date_type) #日期
					self.sort_info(vols,'TradeVolume',stock_text,date_status,date_type) #成交量
					self.sort_info(macd,'MACD9',stock_text,date_status,date_type) #macd
					self.sort_info(rsi,'RSI6',stock_text,date_status,date_type) #rsi
					self.sort_info(k_value,'K9_',stock_text,date_status,date_type) #k_value
					self.sort_info(d_value,'D9',stock_text,date_status,date_type) #d_value
					self.sort_info(dif,'DIF12and26',stock_text,date_status,date_type) #dif
				
			else:
				self.sort_info(close_pr,'ClosingPrice','1101',date_status,date_type) #收盤
				self.sort_info(open_pr,'OpeningPrice','1101',date_status,date_type) #開盤
				self.sort_info(high_pr,'HighestPrice','1101',date_status,date_type) #最高價
				self.sort_info(low_pr,'LowestPrice','1101',date_status,date_type) #最低價
				if date_status == 'DayStockInformation':
					self.sort_info(dates,'TradeDate','1101',date_status,date_type) #日期
				elif date_status == 'WeekStockInformation':
					self.sort_info(dates,'StartDate','1101',date_status,date_type) #日期
				elif date_status == 'MonthStockInformation':
					self.sort_info(dates,'StartDate','1101',date_status,date_type) #日期
				self.sort_info(vols,'TradeVolume','1101',date_status,date_type) #成交量
				self.sort_info(macd,'MACD9','1101',date_status,date_type) #macd
				self.sort_info(rsi,'RSI6','1101',date_status,date_type) #rsi
				self.sort_info(k_value,'K9_','1101',date_status,date_type) #k_value
				self.sort_info(d_value,'D9','1101',date_status,date_type) #d_value
				self.sort_info(dif,'DIF12and26','1101',date_status,date_type) #dif
	##########################################################################################

	#################################以下為資料整理############################################

			for ma_dif in range(0,len(macd)):
				if dif[ma_dif] == '#' or macd[ma_dif] == '#':
					pass
				else:
			...
			sma_5 = talib.SMA(np.array(close_pr,dtype=float), 5) #製作MA5
			sma_20 = talib.SMA(np.array(close_pr,dtype=float), 20) #製作MA20
			sma_60 = talib.SMA(np.array(close_pr,dtype=float), 60) #製作MA60
			H_line,M_line,L_line=talib.BBANDS(np.array(close_pr,dtype=float), timeperiod=20, nbdevup=2, nbdevdn=2, matype=0)#製作布林通道
			...
			bar_macd = (
				...
				.set_global_opts(
					xaxis_opts = opts.AxisOpts(
						is_show=True,
						splitline_opts = opts.SplitLineOpts(is_show=True),
						axislabel_opts = opts.LabelOpts(is_show=True if str(self.comboBox_4.currentText()) == "MACD" else False),
						axistick_opts = opts.AxisTickOpts(is_show=False),
					),
					legend_opts = opts.LegendOpts(is_show=False, pos_bottom=10, pos_left="center"),
					yaxis_opts=opts.AxisOpts(
	...
	#################################以下為圖形繪製################################################
			...
			if self.comboBox_4.currentText() != self.comboBox_3.currentText() and self.comboBox_4.currentText() != self.comboBox_2.currentText() and self.comboBox_4.currentText() != self.comboBox.currentText() and self.comboBox_3.currentText() != self.comboBox_2.currentText() and self.comboBox_3.currentText() != self.comboBox.currentText() and self.comboBox_2.currentText() != self.comboBox.currentText():
				...
				opendata = list()
				closedata = list()
				for opens in open_pr:
					opendata.append(float(opens))
				for close in close_pr:
					closedata.append(float(close))
				grid_chart.add_js_funcs("var openData = {}".format(opendata))
				grid_chart.add_js_funcs("var closeData = {}".format(closedata))
				grid_chart.add_js_funcs("var stock_info = ''")
