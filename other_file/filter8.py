...
def Filter(submit):
    ...
    for I  in submit:
        ...
        if I[0] =='T0' and I[1]=='NewMonth':
            sqls.append(
                "(SELECT stock.sid,stock.s_name,MonthStockInformation.TradingMonth,MonthStockInformation.OpeningPrice,MonthStockInformation.HighestPrice,MonthStockInformation.LowestPrice,MonthStockInformation.ClosingPrice,MonthStockInformation.Change_,MonthStockInformation.Transation_ FROM MonthStockInformation,stock WHERE TradingMonth='{0[2]}' and stock.sid=MonthStockInformation.sid) ".format(I)
            )
        if I[0] =='T0' and I[1]=='NewWeek':
            sqls.append(
                "(SELECT stock.sid,stock.s_name,WeekStockInformation.TradingWeek,WeekStockInformation.OpeningPrice,WeekStockInformation.HighestPrice,WeekStockInformation.LowestPrice,WeekStockInformation.ClosingPrice,WeekStockInformation.Change_,WeekStockInformation.Transation_ FROM WeekStockInformation,stock WHERE TradingWeek='{0[2]}' and stock.sid=WeekStockInformation.sid) ".format(I)
            )
            ...
            if I[0] =='T7' or   I[0] =='T8' or  I[0] =='T9':
                ...
                if I[1] == 'KDGoldenCross' or I[1]=='RSIGoldenCross':
                    if I[4]=='DayStockInformation':
                        extra=" and  CONVERT(REPLACE({0[2]},',',''),DECIMAL(9,2)) BETWEEN CONVERT(REPLACE({0[8][0]},',',''),DECIMAL(9,2)) AND CONVERT(REPLACE({0[8][1]},',',''),DECIMAL(9,2))".format(I)
                ...
                # 死亡交叉
                if I[1] == 'KDDeathCross' or I[1]=='RSIDeathCross':
                    if I[4]=='DayStockInformation':
                        extra=" and  CONVERT(REPLACE({0[2]},',',''),DECIMAL(9,2)) BETWEEN CONVERT(REPLACE({0[8][0]},',',''),DECIMAL(9,2)) AND CONVERT(REPLACE({0[8][1]},',',''),DECIMAL(9,2))".format(I)
                ...
                if I[1]=='Breakupward':
                    if I[4]=='DayStockInformation':
                        ...
                        sqls.append(
                            "(SELECT sid,{0[2]},{0[3]} FROM {0[4]} WHERE {0[8]} {0[2]}<{0[3]} and  TradeDate='{0[6]}') AS {0[5]} INNER JOIN (SELECT sid,{0[2]},{0[3]} FROM {0[4]} WHERE {0[8]} {0[2]}>{0[3]} and TradeDate='{0[7]}')".format(I)
                        )
                    if I[4]=='WeekStockInformation':

                        sqls.append(
                            "(SELECT sid,{0[2]},{0[3]} FROM {0[4]} WHERE {0[8]} {0[2]}<{0[3]} and  TradingWeek='{0[6]}') AS {0[5]} INNER JOIN (SELECT sid,{0[2]},{0[3]} FROM {0[4]} WHERE {0[8]} {0[2]}>{0[3]} and TradingWeek='{0[7]}')".format(I)
                        ) 
                    if I[4]=='MonthStockInformation':

                        sqls.append(
                            "(SELECT sid,{0[2]},{0[3]} FROM {0[4]} WHERE {0[8]} {0[2]}<{0[3]} and  TradingMonth='{0[6]}') AS {0[5]} INNER JOIN (SELECT sid,{0[2]},{0[3]} FROM {0[4]} WHERE {0[8]} {0[2]}>{0[3]} and TradingMonth='{0[7]}')".format(I)
                        ) 
                # 向下突破
...
".format(I)
        ...
        for I in submit:
            ...
            if I[0]=='20MV':
            # D ND LW PW TW
                sql=sql+" 20MV.sid=LW.sid AND 20MV.sid=PW.sid AND 20MV.sid=TW.sid AND (20MV.ClosingPrice>LW.HighestPrice) AND 20MV.ClosingPrice>PW.HighestPrice AND PW.PriceMA60<=TW.PriceMA60 AND LW.PriceMA60>=PW.PriceMA60 AND PW.PriceMA20>TW.PriceMA20 AND LW.PriceMA20>PW.PriceMA20 AND "
            on_count+=1
    ...
    if submit[0][1] == 'NewMonth':
        sql="SELECT T0.* FROM "+sql+" ORDER BY T0.sid;"
    if submit[0][1] == 'NewWeek':
        sql="SELECT T0.* FROM "+sql+" ORDER BY T0.sid;"
    if submit[0][1] == 'RSI':
...
if __name__=='__main__':
    ...
    submit=[
        ...
        ['T0','NewWeek',LastWeek],
    # ['T1', 'DayStockInformation', 'ClosingPrice', '', ('10', '500'), '2021/4/27'], 
    # ['T8', 'RSIGoldenCross', 'RSI6', 'RSI9', 'DayStockInformation', 'Y8', '2021/4/26', '2021/4/27',('-1000000', '1000000'),''],
    #  ['T8', 'Breakupward', 'ClosingPrice', 'PriceMA5', 'DayStockInformation', 'Y8',  '2021/4/26', '2021/4/27', ''],
    # ['T8', 'KDGoldenCross', 'K9_', 'D9', 'DayStockInformation', 'Y8', '2021/4/26', '2021/4/27', ('-1000000', '1000000'), ''],
    # ['T9', 'RSIGoldenCross', 'RSI6', 'RSI9', 'DayStockInformation', 'Y9', '2021/4/26', '2021/4/27', ('-100000', '20'), '']
    # ['T9', 'KDDeathCross', 'RSI6', 'RSI9', 'DayStockInformation', 'Y9', '2021/4/26', '2021/4/27', ('20', '50'), '']
    ['T9', 'KDDeathCross', 'K9_', 'D9', 'DayStockInformation', 'Y9',  '2021/4/26', '2021/4/27', ('50', '80'), '']
    #  ['T10', 'DayStockInformation', 'ClosingPrice', 'DESC', '500', '2021-04-27']
     ]
    ...
    #         # # 成交股數
    #         ['T1', 'DayStockInformation', 'OpeningPrice', '', ('10', '1200'), '2021/4/27'],
    #         # # # # 資券相抵
    ...
    #         ['T8','DayStockInformation','K9_','',('80','610'),LastDay],
    ...
    #         # ,
    #         ['T10','DayStockInformation','ClosingPrice','DESC','100',LastDay],
